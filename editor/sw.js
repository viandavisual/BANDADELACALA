...
self.addEventListener('activate',event=>{
  event.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith('banda-de-la-cala-editor-')&&k!==CACHE).map(k=>caches.delete(k)))).then(()=>self.clients.claim()));
});
