...
Deno.serve(async req => {
  ...
  try {
    ...
    if (action === 'delete') {
      ...
      // v0.48 · Un ADMIN pot eliminar qualsevol altre compte, inclòs un altre ADMIN.
      // Un GESTOR només pot eliminar STANDARD i mai pot eliminar un ADMIN o un altre GESTOR.
      if (targetProfile.role === 'admin' && callerProfile.role !== 'admin') {
        return json({ error: 'ADMIN_DELETE_REQUIRES_ADMIN' }, 403)
      }
      if (callerProfile.role === 'gestor' && targetProfile.role !== 'standard') {
        return json({ error: 'GESTOR_CAN_ONLY_DELETE_STANDARD' }, 403)
      }
