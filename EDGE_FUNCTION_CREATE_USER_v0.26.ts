...
Deno.serve(async req => {
  ...
  try {
    ...
    if (user) {
      // Recuperació d'un compte pendent creat amb v0.25: primer fixem la contrasenya.
      // Canviar la contrasenya invalida OTPs antics, per això la invitació NOVA s'envia DESPRÉS.
      ...
      // v0.26: el password es crea ABANS de generar el token d'invitació.
    ...
    // IMPORTANT: aquesta ha de ser l'última operació Auth que modifiqui tokens.
    // inviteUserByEmail emet un token de confirmació nou i vàlid.
    ...
    if (inviteError) {
      // Si el compte acaba de néixer i ni tan sols hem pogut enviar la invitació,
      // evitem deixar un usuari orfe. Els comptes pendents recuperats es conserven
      // perquè el gestor pugui tornar a intentar l'enviament.
